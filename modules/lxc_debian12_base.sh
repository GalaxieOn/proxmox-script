...
ct_id=$(prompt_input "LXC Debian 12" "ID du conteneur (CT ID)" "")
json_set "$json_file" "ct_id" "$ct_id"

hostname=$(prompt_input "LXC Debian 12" "Hostname" "debian12")
...
template=$(get_debian12_template)
if [[ -z "$template" ]]; then
  echo "Template Debian 12 introuvable via pveam." >&2
  exit 1
fi
...
whiptail --title "Terminé" --msgbox "LXC Debian 12 créé et démarré." 10 60
