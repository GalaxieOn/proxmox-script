...
ct_id=$(prompt_input "LXC Debian 13.3" "ID du conteneur (CT ID)" "")
json_set "$json_file" "ct_id" "$ct_id"

hostname=$(prompt_input "LXC Debian 13.3" "Hostname" "debian13")
...
template=$(get_debian133_template)
if [[ -z "$template" ]]; then
  echo "Template Debian 13.3 introuvable via pveam." >&2
  exit 1
fi
...
whiptail --title "Terminé" --msgbox "LXC Debian 13.3 créé et démarré." 10 60
